...
bool phytium_edp_panel_poweron(struct phytium_dp_device *phytium_dp);
void phytium_edp_panel_poweroff(struct phytium_dp_device *phytium_dp);
...
static struct drm_display_mode tcl_custom_modes_1920[] = {
	/*1920x1200-120hz*/
	{ DRM_MODE("1920x1200", DRM_MODE_TYPE_DRIVER, 319488, 1920, 1968,
			2000, 2080, 0, 1200, 1203, 1209, 1280, 0,
...
static struct drm_display_mode tm_custom_modes_1920[] = {
	/*1920x1200-120hz*/
	{ DRM_MODE("1920x1200", DRM_MODE_TYPE_DRIVER, 314499, 1920, 1968,
			2000, 2080, 0, 1200, 1206, 1212, 1260, 0,
			DRM_MODE_FLAG_NHSYNC | DRM_MODE_FLAG_NVSYNC) },
};

static struct drm_display_mode boe_custom_modes[] = {
	/*1920x1200-120hz*/
	{ DRM_MODE("1920x1200", DRM_MODE_TYPE_DRIVER, 317491, 1920, 1968,
			2000, 2080, 0, 1200, 1203, 1209, 1272, 0,
			DRM_MODE_FLAG_NHSYNC | DRM_MODE_FLAG_NVSYNC) },
};
...
void phytium_wait_time_cycle(ktime_t now, ktime_t last, uint32_t delay_ms)
{
	...
	if (now < last)
		return;
	time_duration = ktime_ms_delta(now, last);
	DRM_DEBUG_KMS("%s time_duration is %lld ms\n", __func__, time_duration);
	if (time_duration < (s64)delay_ms) {
		DRM_DEBUG_KMS("Wait for cycle remain %lld ms\n", delay_ms - time_duration);
		wait_remaining_ms_from_jiffies(jiffies, delay_ms - time_duration);
		}
}
...
phytium_dp_hw_aux_transfer_write(struct phytium_dp_device *phytium_dp, struct drm_dp_aux_msg *msg)
{
	...
	if (interrupt_status & REPLY_TIMEOUT) {
		DRM_DEBUG_KMS("aux(%d) write reply timeout\n", phytium_dp->port);
		ret = -EIO;
		goto out;
	} else if (aux_status & REPLY_ERROR) {
...
phytium_dp_hw_aux_transfer_read(struct phytium_dp_device *phytium_dp, struct drm_dp_aux_msg *msg)
{
	...
	if (interrupt_status & REPLY_TIMEOUT) {
		DRM_DEBUG_KMS("aux(%d) read reply timeout\n", phytium_dp->port);
		ret = -EIO;
		goto out;
	} else if (aux_status & REPLY_ERROR) {
...
static void phytium_dp_aux_init(struct phytium_dp_device *phytium_dp)
{
		...
		phytium_dp->aux.drm_dev = phytium_dp->dev;
#endif
}
#endif
...
static int phytium_add_custom_modes(struct drm_connector *connector, struct edid *edid)
{
	...
	case phytium_edid_encode_panel_id('C', 'S', 'W', 0x1464):
		...
		break;
	case phytium_edid_encode_panel_id('C', 'S', 'W', 0x1463):
		num_mode += phytium_do_add_custom_modes(connector,
						tcl_custom_modes_1920, ARRAY_SIZE(tcl_custom_modes_1920));
		break;
	case phytium_edid_encode_panel_id('T', 'M', 'A', 0x2056):
		num_mode += phytium_do_add_custom_modes(connector,
						tm_custom_modes, ARRAY_SIZE(tm_custom_modes));
		break;
	case phytium_edid_encode_panel_id('T', 'M', 'A', 0x2090):
		num_mode += phytium_do_add_custom_modes(connector,
						tm_custom_modes_1920, ARRAY_SIZE(tm_custom_modes_1920));
		break;
	case phytium_edid_encode_panel_id('B', 'O', 'E', 0x0da1):
		num_mode += phytium_do_add_custom_modes(connector,
						boe_custom_modes, ARRAY_SIZE(boe_custom_modes));
	default:
		break;
	}
	return num_mode;
...
static void get_custom_enable_delay(struct drm_connector *connector, struct edid *edid,
								struct phytium_dp_device *phytium_dp)
{
	uint32_t panel_id = 0;

	if (!edid || !drm_edid_is_valid(edid)) {
		pr_err("edid invalid,skip custom delay\n");
	}

	panel_id = edid_extract_panel_id(edid);

	phytium_dp->custom_delay_ms = 0;
	switch (panel_id) {
	case phytium_edid_encode_panel_id('D', 'E', 'L', 0x41B9):
		phytium_dp->custom_delay_ms = 30;
		break;
	default:
		break;
	}
	FTD330_LOG("%s dp-%d set phytium_dp->custom_delay_ms:%d\n",__func__, phytium_dp->port,phytium_dp->custom_delay_ms);
	return;

}


static void phytium_dp_set_custom_fifo_value(struct phytium_dp_device *phytium_dp, int custom_fifo_value)
{
        struct drm_device *dev =  phytium_dp->dev;
        struct ftd330_drm_private *priv = dev->dev_private;
        int port = phytium_dp->port;
        ...
        phytium_writel_reg(priv, custom_fifo_value,
                           group_offset, PHYTIUM_DP_DATA_CONTROL);
}


static void phytium_dp_custom_fifo_value(struct phytium_dp_device *phytium_dp)
{
        switch (phytium_dp->custom_panel_id) {
                case phytium_edid_encode_panel_id('A', 'O', 'C', 0x2703):
                        if ((phytium_dp->mode.hdisplay == 3840) &&
                                (phytium_dp->mode.vdisplay == 2160) &&
                                ((int)(drm_mode_vrefresh(&phytium_dp->mode)) == 60)) {
                                phytium_dp_set_custom_fifo_value(phytium_dp, 0x20042005);
                        } else {
                                phytium_dp_set_custom_fifo_value(phytium_dp, PHYTIUM_DP_DEFAULT_FIFO_VALUE);
                        }
                        break;
                default:
			phytium_dp_set_custom_fifo_value(phytium_dp, PHYTIUM_DP_DEFAULT_FIFO_VALUE);
...
static int phytium_connector_get_modes(struct drm_connector *connector)
{
	...
	phytium_dp_set_bpc(phytium_dp);
		...
		get_custom_enable_delay(connector, edid, phytium_dp);
...
static void phytium_set_train_link_rate_lane_count(struct phytium_dp_device *phytium_dp,
											struct edid *edid)
{
	uint32_t index = 0;

	if (phytium_dp->num_common_rates <= 0 || 
	...
	DRM_DEBUG_KMS("[DP-%d]: common_max_lane_count: %d, common_max_rate:%d\n",
				phytium_dp->port,
							  ...
							  uint8_t train_pattern)
{
	...
	uint32_t group_offset = priv->dp_reg_base[port];
	bool enable_scrambling = false;

	if (train_pattern == DP_TRAINING_PATTERN_DISABLE) {
		phytium_writel_reg(priv, TRAINING_OFF, group_offset,
				   PHYTIUM_DP_TRAINING_PATTERN_SET);
		phytium_writel_reg(priv, SCRAMBLING_ENABLE, group_offset,
				   PHYTIUM_DP_SCRAMBLING_DISABLE);
		...
		case DP_TRAINING_PATTERN_4:
			...
			break;
		default:
			DRM_INFO("Unknown training pattern '%d'\n", train_pattern);
			tmp = TRAINING_OFF;
			break;
		}
...
static int write_file(struct phytium_dp_device *phytium_dp, const char *filename, const char *data)
{
	struct file *filp;
	loff_t pos = 0;
	...
	if (current->flags & PF_EXITING) {
		pr_err("Process is exiting!\n");
		return -1;
	}

	if (!current->fs) {
		pr_err("current->fs is NULL!\n");
		return -1;
	}

	if (!current->fs->root.dentry || !current->fs->root.mnt) {
		pr_err("root path is invalid!\n");
		return -1;
	}
	...
	if (IS_ERR(filp)) {
		...
		phytium_dp->i2s_control_not_exit = true;
		return PTR_ERR(filp);
	}

	ret = kernel_write(filp, data, strlen(data), &pos);
	if (ret < 0) {
		pr_err("Failed to write file: %ld\n", ret);
...
static void phytium_disable_i2s(struct phytium_dp_device *phytium_dp){
	int physical_display_id = phytium_port_virtual_to_physical(phytium_dp);
	char buffer[100];

	snprintf(buffer, sizeof(buffer), "/sys/devices/platform/PHYT0062:0%d/phyt_i2s_control", physical_display_id);

	if (write_file(phytium_dp, buffer, "0") == 0)
		phytium_dp->i2s_control_switch = true;
}

static void phytium_enable_i2s(struct phytium_dp_device *phytium_dp){
	int physical_display_id = phytium_port_virtual_to_physical(phytium_dp);
	char buffer[100];

	snprintf(buffer, sizeof(buffer), "/sys/devices/platform/PHYT0062:0%d/phyt_i2s_control", physical_display_id);

	if (write_file(phytium_dp, buffer, "1") == 0)
		phytium_dp->i2s_control_switch = false;
}

void phytium_dp_hw_disable_audio(struct phytium_dp_device *phytium_dp)
{
	if (!phytium_dp->i2s_control_not_exit)
		phytium_disable_i2s(phytium_dp);
}

bool phytium_dp_hw_audio_is_enable(struct phytium_dp_device *phytium_dp)
{
	struct drm_device *dev =  phytium_dp->dev;
	struct ftd330_drm_private *priv = dev->dev_private;
	...
	return config == SEC_AUDIO_ENABLE ? true : false;
}
...
void phytium_dp_hw_enable_audio(struct phytium_dp_device *phytium_dp)
{
	...
	if (phytium_dp->i2s_control_switch && !phytium_dp->i2s_control_not_exit)
		phytium_enable_i2s(phytium_dp);
}
...
void phytium_dp_hw_config_video(struct phytium_dp_device *phytium_dp)
{
	...
	phytium_dp_custom_fifo_value(phytium_dp);
...
int phytium_dp_start_link_train(struct phytium_dp_device *phytium_dp, bool sink_needs_dpms)
{
	...
	if (!phytium_dp_link_training_clock_recovery(phytium_dp)) {
	        /* we need skip link_train to avoid DP send message to a
	         * disconnected monitor, which causes aux read error.
		*/
		if ((phytium_dp->connector.status == connector_status_disconnected) &&
				!phytium_readl_reg(priv, group_offset, PHYTIUM_DP_SINK_HPD_STATE)) {
			if (drm_dp_dpcd_readb(&phytium_dp->aux, DP_SINK_COUNT, &sink_count) < 0) {
				return 0;
			}
		}
	...
	DRM_DEBUG_KMS("[CONNECTOR:%d:%s] Full Link Training Pass at Link Rate = %d, Lane count = %d\n",
...
static int phytium_dp_long_pulse(struct drm_connector *connector, bool hpd_raw_state)
{
	...
	if (!phytium_dp->is_edp) {
		...
		phytium_set_train_link_rate_lane_count(phytium_dp, edid);
		phytium_dp->max_pass_link_rate = 0;
		phytium_dp->max_pass_lane_count = 0;
...
void phytium_display_power_request_on(struct drm_device *dev, int physical_display_id, bool handle_irq)
{
	...
	if (physical_display_id == DISPLAY_0) {
		...
		mutex_lock(&priv->power_mutex);
		if (priv->phytium_dp[physical_display_id]->is_edp) {
			priv->phytium_dp[physical_display_id]->edp_enable_status = true;
		}
...
void phytium_display_power_request_off(struct drm_device *dev, int physical_display_id)
{
	...
	if (physical_display_id == DISPLAY_0) {
		if (priv->info.edp_mask & BIT(DISPLAY_0)) {
			...
			if (connector_status[DISPLAY_0] != connector_status_connected) {
				if (priv->info.pipe_mask & BIT(DISPLAY_0)) {
					phytium_dp_hw_hpd_irq_setup(priv->phytium_dp[DISPLAY_0], false, false);
				}
#ifdef CONFIG_PHYTIUM_POWER_OPERATION
		...
		if (priv->phytium_dp[physical_display_id]->is_edp) {
			priv->phytium_dp[physical_display_id]->edp_enable_status = false;
		}
		...
		if ((physical_display_id == DISPLAY_1) &&
			!(priv->info.pipe_mask & BIT(DISPLAY_2)) &&
			((priv->phytium_dp[DISPLAY_1]->is_edp && !priv->phytium_dp[DISPLAY_1]->edp_enable_status) ||
		 	 (priv->phytium_dp[DISPLAY_1]->connector.status != connector_status_connected))) {
		...
		if ((physical_display_id == DISPLAY_2) &&
			...
			((priv->phytium_dp[DISPLAY_2]->is_edp && !priv->phytium_dp[DISPLAY_2]->edp_enable_status) || (priv->phytium_dp[DISPLAY_2]->connector.status != connector_status_connected))) {
		...
		if (priv->info.pipe_mask & BIT(DISPLAY_1) && priv->info.pipe_mask & BIT(DISPLAY_2)) {
			if (priv->phytium_dp[DISPLAY_1]->is_edp && priv->phytium_dp[DISPLAY_2]->connector.status != connector_status_connected && !priv->phytium_dp[DISPLAY_1]->edp_enable_status) {
				power_down = true;
			}

			if (priv->phytium_dp[DISPLAY_2]->is_edp && priv->phytium_dp[DISPLAY_1]->connector.status != connector_status_connected && !priv->phytium_dp[DISPLAY_2]->edp_enable_status) {
				power_down = true;
			}

			if ((priv->phytium_dp[DISPLAY_1]->connector.status != connector_status_connected) && (priv->phytium_dp[DISPLAY_2]->connector.status != connector_status_connected)) {
				power_down = true;		
			}

			if (power_down && !priv->phytium_dp[DISPLAY_1]->dp_power_enable_status &&
...
bool phytium_dp_fast_link_train(struct phytium_dp_device *phytium_dp)
{
	...
	DRM_DEBUG_KMS("[CONNECTOR:%d:%s] Fast Link Training Pass at Link Rate = %d, Lane count = %d\n",
		      phytium_dp->connector.base.id,
		      phytium_dp->connector.name, phytium_dp->link_rate,
		      phytium_dp->link_lane_count);
...
/*we do link train here.so we can */
	...
	if (!phytium_dp->is_edp) {
		if ((status == connector_status_disconnected) && hpd_raw_state) {
			status = connector_status_connected;
		}
	}
...
#endif
	phytium_dp->aux.dev = phytium_dp->connector.kdev;
	drm_dp_aux_register(&phytium_dp->aux);
...
bool phytium_edp_panel_poweron(struct phytium_dp_device *phytium_dp)
{
	...
	success = phytium_panel_poweron(&phytium_dp->panel);
...
void phytium_edp_delay_on_work_func(struct work_struct *work)
{
	struct delayed_work *dwork = to_delayed_work(work);
	struct phytium_dp_device *phytium_dp =
                container_of(dwork, struct phytium_dp_device, edp_delay_on_work);
	int ret = 0;
        bool success = false;
#ifdef CONFIG_PHYTIUM_PSR
        struct drm_device *dev = phytium_dp->dev;
	 ...
	 * proceed with the fast training process.
	 */
	if (!success && phytium_dp->fast_train_support)
...
static void phytium_encoder_enable(struct drm_encoder *encoder)
{
	...
	if (!priv->fb_initialized && phytium_dp->is_edp) {
		phytium_dp->edp_delay_on_work_pending = true;
		schedule_delayed_work(&phytium_dp->edp_delay_on_work,msecs_to_jiffies(5));
		return;
	}
...
static int phytium_dp_audio_get_eld(struct device *dev, void *data, u8 *buf, size_t len)
{
	...
	// only 2-channels is supported
	phytium_dp->connector.eld[DRM_ELD_SPEAKER] = 0;
...
static int phytium_dp_audio_codec_init(struct phytium_dp_device *phytium_dp)
{
	...
	int physical_display_id = phytium_port_virtual_to_physical(phytium_dp);
	int codec_id = PHYTIUM_DP_AUDIO_ID + physical_display_id;
	...
	phytium_dp->i2s_control_switch = false;
	phytium_dp->i2s_control_not_exit = false;
...
int phytium_dp_init(struct drm_device *dev, int port)
{
	...
	int ret = 0;
	int type;
	int get_panel_info_nums = 0;
	bool edp_connected_status = true;
...
reinitialize_edp:
	if (phytium_dp->is_edp) {
		edp_connected_status = phytium_readl_reg(priv, priv->dp_reg_base[phytium_dp->port], PHYTIUM_DP_SINK_HPD_STATE);
		...
		*/
		...
		phytium_dp_hw_init(phytium_dp);
		phytium_edp_panel_poweron(phytium_dp);
		/*before we read hpd state.we need to wait 200 time.cause sometimes after DP
		 * init.HPD is low.and screen need little time to recover HPD*/
		if (!edp_connected_status)
		{
			mdelay(200);
		/* sometimes cuntomers don't connect edp screen.but describe it acpi
		 * so we need to recongnize it and reinitialize it. 
		 * */
			if (!phytium_readl_reg(priv, priv->dp_reg_base[phytium_dp->port], PHYTIUM_DP_SINK_HPD_STATE)) {
				phytium_edp_panel_poweroff(phytium_dp);
				phytium_dp->is_edp = false;
				priv->info.edp_mask &= ~BIT(phytium_port_virtual_to_physical(phytium_dp));
				printk("FTD330 edp_mask is %d\n",priv->info.edp_mask);
				goto reinitialize_edp;
			}
		}
...
get_panel_info:
		...
		if (!phytium_dp->edp_edid && get_panel_info_nums < 3) {
			DRM_ERROR("get edp edid failed\n");

			phytium_edp_panel_poweroff(phytium_dp);
			phytium_edp_panel_poweron(phytium_dp);
			get_panel_info_nums += 1;
			goto get_panel_info;
		}

		if (phytium_dp->edp_edid && drm_edid_is_valid(phytium_dp->edp_edid)) {
...
#endif
	if (phytium_dp->is_edp) {
		INIT_DELAYED_WORK(&phytium_dp->edp_delay_on_work, phytium_edp_delay_on_work_func);
	}
